...
const Dashboard = () => {
  ...
  const totals = {
    ...
    "in progress": data?.tasks?.["in progress"] || 0,
    todo: data?.tasks?.todo || 0,
  };

  const stats = [
    { _id: "1", label: "TOTAL TASK", total: data?.totalTasks || 0, icon: <FaNewspaper />, bg: "bg-[#1d4ed8]" },
    { _id: "2", label: "COMPLETED TASK", total: totals["completed"], icon: <MdAdminPanelSettings />, bg: "bg-[#0f766e]" },
    { _id: "3", label: "TASK IN PROGRESS", total: totals["in progress"], icon: <LuClipboardEdit />, bg: "bg-[#f59e0b]" },
    { _id: "4", label: "TODOS", total: totals["todo"], icon: <FaArrowsToDot />, bg: "bg-[#be185d]" },
  ];
