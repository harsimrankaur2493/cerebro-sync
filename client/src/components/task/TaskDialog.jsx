...
const TaskDialog = ({ task }) => {
  ...
  const duplicateHandler = async () => {
    try {
      // Ensure `_id` is passed as a string to avoid issues with MongoDB's ObjectId
      const res = await duplicateTask({ id: task._id.toString() }).unwrap();
  ...
  const deleteHandler = async () => {
    try {
      const res = await deleteTask({
        id: task._id.toString(), // Explicitly converting `_id` to a string
        isTrashed: "trash",
      }).unwrap();
      toast.success(res?.message);
